...
void init_w(int n)
{
    ...
    for (int d = 2; d <= N; d <<= 1)
    {
        int m = d >> 1;
        w[m] = 1;
        long long g = fast_pow(G, (MOD - 1) / d);
        for (int i = 1; i < m; i++)
            w[i + m] = w[i + m - 1] * g % MOD;
    }
    for (int i = 1; i < N; i++)
...
void Solve(int l, int r)
{
    ...
    poly X, Y;
    for (int i = l; i <= mid; i++)
        X.push_back(pw2[s[max(min(n, i), 0)]]);
    for (int i = mid + 1; i <= r; i++)
        Y.push_back(inv_pw2[s[max(min(n, i), 0)]]);
    X = X * Y;
    for (int i = 0; i < X.size(); i++)
        if (i + l + (mid + 1) >= 0 && i + l + (mid + 1) <= 2 * n)
            c[i + l + (mid + 1)] = add(c[i + l + (mid + 1)], X[i]);
}
...
int main()
{
    ...
    for (int i = 1; i <= m; i++)
        scanf("%d", &p[i]), s[p[i]] = 1;
    for (int i = 1; i <= n; i++)
        s[i] += s[i - 1]; // for(int i=1;i<=n;i++)printf("%d ",s[i]);puts("");
    pw2[0] = 1;
    for (int i = 1; i <= n; i++)
        pw2[i] = add(pw2[i - 1], pw2[i - 1]);
    inv_pw2[n] = INV(pw2[n]);
    for (int i = n - 1; i >= 0; i--)
        inv_pw2[i] = add(inv_pw2[i + 1], inv_pw2[i + 1]);
    Solve(-n, 2 * n);
    // for(int i=0;i<=2*n;i++)printf("%lld ",c[i]*pw2[m]%MOD);puts("");
    long long ans = 0;
    for (int i = 1; i <= n; i++)
        ans = add(ans, sub(pw2[m] * c[2 * i - 1] % MOD, min(i + n, 2 * n - i + 1)) * a[i] % MOD);
